...
const indeterminantInteractiveElementsMap = domElements.reduce((accumulator: {
  [key: string]: Array<any>
}, name: string): { [key: string]: Array<any> } => {
  const map = { ...accumulator, [name]: [] };
  return map;
}, {});
  ...
  return (
    ...
    (openingElement.attributes.length > 0
      ? `${openingElement.attributes
          .map(attr => `[${attr.name.name}=\"${attr.value.value}\"]`) // eslint-disable-line no-useless-escape
          .join('')}`
      : '')
  );
}
